...
switch (rcvd.df1_stuff[9])
	...
	case 0xee:		// SLC 5/05
		...
		plc5status.error_word = (rcvd.df1_stuff[21]*256) + rcvd.df1_stuff[20];
		plc5status.mode = (rcvd.df1_stuff[22] && 31);
		plc5status.forces_on = (rcvd.df1_stuff[22] & 32);
		plc5status.forces_here = (rcvd.df1_stuff[22] & 64);
		plc5status.comm_active = (rcvd.df1_stuff[22] & 128);
		plc5status.protection_power_loss = (rcvd.df1_stuff[23] & 1);
		plc5status.startup_protect_fault = (rcvd.df1_stuff[23] & 2);
		plc5status.load_rom_on_error = (rcvd.df1_stuff[23] & 4);
		plc5status.load_rom_always = (rcvd.df1_stuff[23] & 8);
		plc5status.load_rom_and_run = (rcvd.df1_stuff[23] & 16);
		plc5status.plc_halted = (rcvd.df1_stuff[23] & 32);
		plc5status.locked = (rcvd.df1_stuff[23] & 64);
		plc5status.first_scan = (rcvd.df1_stuff[23] & 128);
		plc5status.program_id = (rcvd.df1_stuff[25]*256) + rcvd.df1_stuff[24];
		plc5status.memsize = rcvd.df1_stuff[26];
		plc5status.mem_bad = (rcvd.df1_stuff[27] & 1);
		plc5status.program_owner = rcvd.df1_stuff[27]/4;
		return plc5status;
	}
return plc5status;
}	
